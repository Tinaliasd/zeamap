...
public class TfbdInfoController extends BaseController {
    ...
    public TableDataInfo selectInfo(InfoSelectCondition selectCondition) {
        startPage();
...
*/
    ...
    public void download(HttpServletResponse response, @RequestParam List<InfoSelectCondition> list) {
        try {
            List<InfoSelectCondition> Infolist = new ArrayList<>();
            for (InfoSelectCondition info:list) {
                InfoSelectCondition selectCondition = new InfoSelectCondition(info.getAnalysis_name(),info.getRef(),info.getInfo_name(),info.getInfo_simplename(),info.getInfo_family(),info.getTarget_gene(),info.getP_value());
                Infolist.add(selectCondition);
            }
            ExcelUtil<InfoSelectCondition> util = new ExcelUtil<>(InfoSelectCondition.class);
            util.exportExcel(response,Infolist, "查询数据 info_table");
        } catch (Exception ex) {
            logger.error("下载info异常", ex);
        }
    }
