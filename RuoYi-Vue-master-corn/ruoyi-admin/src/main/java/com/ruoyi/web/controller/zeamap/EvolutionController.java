...
public class EvolutionController extends BaseController {
    ...
    public TableDataInfo selectevol(EvolSelectCondition evolSelectCondition) {
        startPage();
    ...
    public void download(HttpServletResponse response, @RequestParam List<EvolSelectCondition> list) {
        try {
            List<EvolSelectCondition> evollist = new ArrayList<>();
            for (EvolSelectCondition evol : list) {
                EvolSelectCondition selectCondition = new EvolSelectCondition(evol.getRef(), evol.getVersion(), evol.getAnalysis_name(), evol.getIndicator(), evol.getSelect(), evol.getCompare_pop(), evol.getChro(), evol.getStart(), evol.getEnd(), evol.getValue(),evol.getTag());
                evollist.add(selectCondition);
            }
            ExcelUtil<EvolSelectCondition> util = new ExcelUtil<>(EvolSelectCondition.class);
            util.exportExcel(response, evollist, "查询数据 evoltion");
        } catch (Exception ex) {
