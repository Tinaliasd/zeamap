...
public class ChromatinInteractionController extends BaseController
{
    ...
    *
    ...
    @GetMapping("/selectchromatin_interaction")
    public TableDataInfo selectchromatin_interaction(ChromatinInteractionSelectCondition chromatinInteractionSelectCondition) {
        startPage();
    ...
    @Log(title = "【下载chromatin_interaction数据】", businessType = BusinessType.EXPORT)
    @PostMapping("/download")
    public void download(HttpServletResponse response, @RequestBody String jsonString) {
        //JSON字符串转List<ChromatinInteraction>
        System.out.println("+++++++++++:"+jsonString);
        List<ChromatinInteraction> list=new ArrayList<ChromatinInteraction>(JSON.parseArray(jsonString,ChromatinInteraction.class));
        ExcelUtil<ChromatinInteraction> util = new ExcelUtil<ChromatinInteraction>(ChromatinInteraction.class);
        util.exportExcel(response,list, "查询chromatin_interaction数据");
    }
     */
